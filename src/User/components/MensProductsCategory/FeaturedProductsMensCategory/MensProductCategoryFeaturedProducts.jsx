// MensProductCategoryFeaturedProducts.js
import React, { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { ChevronLeftIcon, ChevronRightIcon } from "@heroicons/react/24/outline";
import getMenProducts from '../../../../Data/MenSectionHomeData'; // Updated import
import FeaturedProductsCarousel from './FeaturedProductsCarousel';

const MensProductCategoryFeaturedProducts = () => {
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isTransitioning, setIsTransitioning] = useState(true);
  const [itemsPerView, setItemsPerView] = useState(4);
  const [featuredProducts, setFeaturedProducts] = useState([]);

  // Load featured products - updated to use getMenProducts directly
  useEffect(() => {
    const productsData = getMenProducts;
  ...
  // If no valid data, render fallback
  if (featuredProducts.length === 0) {
    return (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Discover our handpicked selection of premium men's products
            </p>
          </div>
...
export default MensProductCategoryFeaturedProducts;